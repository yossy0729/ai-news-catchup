...
- 出典は Paper / Code / PwC（掲載元）。PwC未登録の分野は数値を出さずリンクのみ掲載します（誤値で信頼を損なわないため）。

関連ファイル: `data/sota.json`（生成物・履歴保持）/ `data/sota-labels.json`（日本語辞書）/ `data/sota-presets.json`（前線プリセット）。`data/cache/` はAPIキャッシュで版管理対象外です。
